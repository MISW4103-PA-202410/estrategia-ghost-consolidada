    ...
    cy.window().then((win)=>{
        ...
        if($inputs.length > 0){
            ...
            if(!Cypress.dom.isHidden(inp)) {
                focused = true;
                cy.task("logCommand", {funtype: "input " + inp.getAttribute("type"), info: `Input ${inp.id} is focused`})
...
function randomEvent(){
    ...
    if(pending_events[typeIndex] > 0){
        ...
        //cy.task("logCommand", { funtype: "Next fucntion: "+functions[typeIndex][fIndex], info: ""})
...
function getEvtType(i){
    ...
    else if (i===1) return "Selector focus"
    else if (i===2) return "Keypress"
    else if (i===3) return "Special Keypress"
    else if (i===4) return "Page Navigation"
    //else if (i===5) return "Browser Chaos"
    else if (i===5) return "Action/Click"
}
...
describe( `${appName} under smarter monkeys`, function() {
    ...
    it(`visits ${appName} and survives smarter monkeys`, function() {
        ...
        if(pcg === 100){
            //bool que pregunte si featrues esta en la lsita preestablecida
            ...
            if(featureValido){
                ...
                //pending_events[5] = events*pct_browserChaos/100;
                pending_events[5] = events*pct_actions/100;
                const urlFeature  = url + feature;
                cy.visit(urlFeature).then((win)=>{
                    let d = win.document
                    curPageMaxY = Math.max( d.body.scrollHeight, d.body.offsetHeight, d.documentElement.clientHeight, d.documentElement.scrollHeight, d.documentElement.offsetHeight) - win.innerHeight
                    curPageMaxX = Math.max( d.body.scrollWidth, d.body.offsetWidth, d.documentElement.clientWidth, d.documentElement.scrollWidth, d.documentElement.offsetWidth) - win.innerWidth
                })
                cy.wait(40000)
                //Log in
                login("equipo20@misw4103.com","PRUEBAS12345");
                cy.wait(10000)
                //Add an event for each type of event in order to enter the else statement of randomEvent method
                for(let i = 0; i < events + 6; i++){
                    evtIndex++
                    randomEvent()
                    //Preguntar si me sali de la feature
                    cy.url().then((url)=>{
                    ...
                    //Preguntar si la url tiene sigin
                    cy.url().then((url)=>{
                ...
                cy.task('genericLog', {'message':`The feature ${features} is not valid`})
            }
        }
...
function login(username, password) {
    cy.get('input[name="identification"]').should('be.visible').type(username);
